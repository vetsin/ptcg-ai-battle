...
**Status (2026-06-19, first pass)**: Implemented in `agent/search.py` (`load_value_model`, `_static_value_eval`, gated behind `_value_net_enabled`, default off). Head-to-head eval (`eval_value_net.py`) of the pre-curriculum `ab_training/final_model.pt` vs the heuristic-rollout baseline across 30 games over all 16 competitive decks: **value-net MCTS lost, 40% WR vs baseline's 60%**. Left disabled. Suspected cause: the value head was trained on self-play games using `slightly_smart_agent` (no MCTS) as the policy, so it learned a shallower notion of "good state" than `evaluate_state` already encodes under full MCTS search.

**Status (2026-06-19, retest after A.4/A.5/C.3 retrain)**: Re-ran the same head-to-head with the curriculum + archetype-feature + matchup-weighted champion. Result flipped: **value-net MCTS won, ~59% WR (47/80 across two 40-game batches)** vs the heuristic baseline. However, a separate check (run_game vs `random_agent`, 20 games each) found a serious regression on the *other* target metric: **vs random dropped to 10% (2/20) with the value net on, vs 35% (7/20) with it off** — tuning `_value_net_blend` down (0.5 -> 0.25 -> 0.15) did not recover this. Root cause is likely structural, not a tuning problem: `_static_value_eval` fully replaces the old 5-step heuristic rollout (which simulated forward against a 70%-heuristic/30%-random opponent model) with a single current-state evaluation. That forward simulation was specifically valuable against unpredictable opponents; a static eval loses it. Net effect: better tactics vs skilled/competitive decks, worse vs random/weak play.

**Decision**: shipped anyway — wired `load_value_model()` into `agent/main.py::_ensure_models_loaded()` (called lazily on first `agent()` invocation, alongside `load_policy_model()`, neither of which was previously invoked anywhere in the real `main.py` entrypoint — a separate bug, see below). Bet is that real Kaggle opponents are closer to competitive-deck skill than to pure random. If live results disagree, the fix is either reverting this wiring (`_value_net_enabled` defaults to off, so just don't call `load_value_model()` in `_ensure_models_loaded()`) or implementing the "hybrid" alternative that was considered but not built: blend the net's evaluation *in addition to* the rollout rather than instead of it, to keep the lookahead that helps vs random.

**Separate bug found and fixed in the same pass**: neither `load_policy_model()` nor `load_value_model()` was ever called anywhere in the actual agent runtime path (`agent/main.py` -> `main.py`, the real Kaggle entrypoint) — only `smoke_test.py` called `load_policy_model()` explicitly, as a side effect of testing the loader itself, which happened to also make smoke_test's *other* tests (full game, vs random) exercise the model purely because of Python's persistent module state within one test process. In the actual competition harness (fresh process, no test-5-style explicit load), the trained model would never have activated at all, regardless of how much training/tuning happened. Fixed by lazy-loading both models on first `agent()` call.
